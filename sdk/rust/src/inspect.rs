//! 轻量 BidRequest / BidResponse Inspect 流水线（匹配 / 出价 / 响应决策）。
//!
//! 基于 OpenRTB JSON（[`serde_json::Value`]），**不**运行完整 JSON Schema——
//! 权威校验请用 [`crate::validate`]。
//!
//! 核心概念：
...
pub struct RequestSharedView<'a> {
    ...
    /// 当 `tmax > 0` 时：inspect 开始时刻 + 85% × tmax。
...
/// [`inspect_request`] 的输出（共享视图 + Imp 列表）。
#[derive(Clone, Debug)]
...
/// 廉价结构检查（非完整 JSON Schema）；通过后再调用 [`pin_request_shared`] 等。
pub fn light_gate_request(req: &Value) -> std::result::Result<(), String> {
    let obj = req
        .as_object()
        .ok_or_else(|| "inspect: BidRequest must be a JSON object".to_string())?;
    let id = obj
        .get("id")
        .and_then(|v| v.as_str())
        .filter(|s| !s.is_empty())
        .ok_or_else(|| "inspect: BidRequest.id is required".to_string())?;
    let _ = id;
    let at = obj.get("at").and_then(|v| v.as_i64()).unwrap_or(0);
    if at == 0 {
        return Err("inspect: BidRequest.at is required".into());
    }
    let cur = obj.get("cur").and_then(|v| v.as_array());
    if cur.map(|a| a.is_empty()).unwrap_or(true) {
        return Err("inspect: BidRequest.cur is required".into());
    }
    let imps = obj
        .get("imp")
        .and_then(|v| v.as_array())
        .ok_or_else(|| "inspect: BidRequest.imp requires at least one Imp".to_string())?;
    if imps.is_empty() {
        return Err("inspect: BidRequest.imp requires at least one Imp".into());
    }
    let n = [
    ...
    .count();
    if n > 1 {
        return Err("inspect: site/app/dooh are mutually exclusive".into());
    }
    for (i, imp) in imps.iter().enumerate() {
        let id = imp.get("id").and_then(|v| v.as_str()).unwrap_or("");
        if id.is_empty() {
            return Err(format!("inspect: imp[{i}].id is required"));
        }
...
/// LightGate 通过后构建 [`RequestResult`]（借用 `req` 内部数据）。
///
/// 一次性编排请用 [`run_request`] 获取 [`RequestSnapshot`]。
pub fn inspect_request(req: &Value) -> std::result::Result<RequestResult<'_>, String> {
    light_gate_request(req)?;
    Ok(RequestResult {
        shared: pin_request_shared(req),
        imps: view_imps(req)?,
    })
}

/// LightGate 之后：构建拍卖级 [`RequestSharedView`]。
pub fn pin_request_shared(req: &Value) -> RequestSharedView<'_> {
...
/// [`pin_request_shared`] 之后：构建各 Imp 的 [`ImpView`] 列表。
pub fn view_imps(req: &Value) -> std::result::Result<Vec<ImpView<'_>>, String> {
    let imps = req
        .get("imp")
        .and_then(|v| v.as_array())
        .ok_or_else(|| "inspect: missing imp".to_string())?;
    let mut out = Vec::with_capacity(imps.len());
    for imp in imps {
        let obj = imp.as_object().ok_or_else(|| "inspect: imp not object".to_string())?;
        out.push(ImpView {
            imp,
            id: obj.get("id").and_then(|v| v.as_str()).unwrap_or(""),
...
/// BidRequest Inspect 流水线：LightGate → pin_shared → view_imps。
///
...
impl<'a> RequestPipeline<'a> {
    ...
    /// 步骤 2：固定拍卖级共享视图。
    pub fn pin_shared(mut self) -> Self {
        if self.err.is_some() {
            return self;
        }
        if !self.gated {
        ...
        self.shared = Some(pin_request_shared(self.req));
        self.pinned = true;
        self
    }

    /// 步骤 3：构建 Imp 视图列表。
    pub fn view_imps(mut self) -> Self {
        if self.err.is_some() {
            return self;
        }
        if !self.pinned {
            self.err = Some("pipeline: call pin_shared first".into());
            return self;
        }
        match view_imps(self.req) {
            Ok(imps) => {
                self.imps = imps;
                self.viewed = true;
            }
...
pub fn light_gate_response(res: &Value) -> std::result::Result<(), String> {
    let obj = res
        .as_object()
        .ok_or_else(|| "inspect: BidResponse must be a JSON object".to_string())?;
    let id = obj.get("id").and_then(|v| v.as_str()).unwrap_or("");
    if id.is_empty() {
        return Err("inspect: BidResponse.id is required".into());
    }
    let cur = obj.get("cur").and_then(|v| v.as_str()).unwrap_or("");
    if cur.is_empty() {
        return Err("inspect: BidResponse.cur is required".into());
    }
    let Some(seatbids) = obj.get("seatbid").and_then(|v| v.as_array()) else {
        return Ok(());
    };
    ...
    for (i, sb) in seatbids.iter().enumerate() {
        let bids = sb
            .get("bid")
            .and_then(|v| v.as_array())
            .ok_or_else(|| format!("inspect: seatbid[{i}] needs at least one bid"))?;
        if bids.is_empty() {
            return Err(format!("inspect: seatbid[{i}] needs at least one bid"));
        }
...
pub fn pin_response_shared(res: &Value) -> ResponseSharedView<'_> {
...
fn view_bid<'a>(bid: &'a Value, seat: Option<&'a str>) -> std::result::Result<BidView<'a>, String> {
    let obj = bid
        .as_object()
        .ok_or_else(|| "inspect: bid not object".to_string())?;
...
/// LightGate + view + pin，一次性 inspect BidResponse。
pub fn inspect_response(res: &Value) -> std::result::Result<ResponseResult<'_>, String> {
    light_gate_response(res)?;
    let (seatbids, bids) = view_seatbids(res)?;
    Ok(ResponseResult {
        shared: pin_response_shared(res),
        seatbids,
        bids,
    })
}

/// BidResponse Inspect 流水线：light_gate → pin_shared → view_bids。
...
impl<'a> ResponsePipeline<'a> {
    ...
    pub fn pin_shared(mut self) -> Self {
        ...
        self.shared = Some(pin_response_shared(self.res));
        self.pinned = true;
        self
    }

    /// 步骤 3：构建 SeatBid / Bid 视图。
    pub fn view_bids(mut self) -> Self {
        if self.err.is_some() {
            return self;
        }
        if !self.pinned {
...
/// 一次性运行 BidResponse 全部 inspect 步骤。
...
mod tests {
    ...
    fn inspect_banner_request() {
        ...
        let res = inspect_request(&req).expect("inspect");
    ...
    fn request_pipeline_step_order_error() {
        ...
        let err = RequestPipeline::of(&req).pin_shared().snapshot().unwrap_err();
    ...
    fn response_pipeline_step_order_error() {
        ...
        let err = ResponsePipeline::of(&res).pin_shared().snapshot().unwrap_err();
        assert!(err.contains("light_gate"));
    }
