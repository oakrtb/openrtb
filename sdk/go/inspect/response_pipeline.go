...
func RunResponse(res *openrtb.BidResponse) (*ResponseSnapshot, error) {
	return OfResponse(res).LightGate().PinShared().ViewBids().Snapshot()
}
...
// PinShared 为第 2 步：构建拍卖级 ResponseSharedView。
func (p *ResponsePipeline) PinShared() *ResponsePipeline {
	if p.err != nil {
		return p
	}
	if !p.gated {
	...
	p.shared = pinResponseShared(p.res)
	p.pinned = true
	return p
}

// ViewBids 为第 3 步：构建 SeatBidView 与扁平 BidView 列表。
func (p *ResponsePipeline) ViewBids() *ResponsePipeline {
	if p.err != nil {
		return p
	}
	if !p.pinned {
