...
func RunRequest(req *openrtb.BidRequest) (*RequestSnapshot, error) {
	return OfRequest(req).LightGate().PinShared().ViewImps().Snapshot()
}
...
// PinShared 为第 2 步：构建拍卖级 RequestSharedView。
func (p *RequestPipeline) PinShared() *RequestPipeline {
	if p.err != nil {
		return p
	}
	if !p.gated {
	...
	p.shared = pinShared(p.req)
	p.pinned = true
	return p
}

// ViewImps 为第 3 步：构建带 markup 位掩码的 ImpView 列表。
func (p *RequestPipeline) ViewImps() *RequestPipeline {
	if p.err != nil {
		return p
	}
	if !p.pinned {
		p.err = fmt.Errorf("pipeline: call PinShared first")
		return p
	}
...
// PastDeadline 报告 inspect 预算是否已超时。
