...
// ResponseResult 是 BidResponse 的检查输出。
type ResponseResult struct {
	Shared   ResponseSharedView
	SeatBids []SeatBidView
	Bids     []BidView // 跨 seat 扁平化
}
...
func LightGateResponse(res *openrtb.BidResponse) error {
	if res == nil {
		return fmt.Errorf("inspect: nil BidResponse")
	}
	if res.Id == "" {
		return fmt.Errorf("inspect: BidResponse.id is required")
	}
	if res.Cur == "" {
		return fmt.Errorf("inspect: BidResponse.cur is required")
	}
	...
	for i, sb := range res.Seatbid {
		if sb == nil {
			return fmt.Errorf("inspect: seatbid[%d] is nil", i)
		}
		if len(sb.Bid) == 0 {
			return fmt.Errorf("inspect: seatbid[%d] needs at least one bid", i)
		}
		for j, bid := range sb.Bid {
			if bid == nil {
				return fmt.Errorf("inspect: seatbid[%d].bid[%d] is nil", i, j)
			}
			if bid.Id == "" || bid.Impid == "" {
				return fmt.Errorf("inspect: seatbid[%d].bid[%d] requires id and impid", i, j)
			}
			if bid.Price <= 0 {
				return fmt.Errorf("inspect: seatbid[%d].bid[%d].price must be > 0", i, j)
			}
		}
	}
	return nil
...
// InspectResponse 先执行 LightGateResponse，再构建响应视图。
func InspectResponse(res *openrtb.BidResponse) (*ResponseResult, error) {
	if err := LightGateResponse(res); err != nil {
		return nil, err
	}
	shared := pinResponseShared(res)
	seats, bids := viewSeatBids(res)
	return &ResponseResult{Shared: shared, SeatBids: seats, Bids: bids}, nil
}

func pinResponseShared(res *openrtb.BidResponse) ResponseSharedView {
