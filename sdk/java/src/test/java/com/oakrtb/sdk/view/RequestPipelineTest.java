...
class RequestPipelineTest {
  ...
  void stepOrderEnforced() {
    ...
    assertThrows(IllegalStateException.class, () -> RequestPipeline.of(req).shared());
  ...
  void sharedPinOnce() throws Exception {
    BidRequest req =
        BidRequestBuilder.create("x")
            ...
            .addImp(ImpBuilders.banner("1").size(1, 1).build())
            .build();
    RequestPipeline p = RequestPipeline.of(req).lightGate().shared();
    Instant d1 = p.snapshot().shared().deadline();
    Thread.sleep(3);
    p.shared();
    Instant d2 = p.snapshot().shared().deadline();
    assertEquals(d1, d2);
  }
}
